...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-20 px-2 sm:px-6 py-1 flex justify-between items-center transition duration-300 ${
    ...
    >
      <div className="flex items-center space-x-2 ml-2 sm:ml-[30px]">
        <img
          ...
          className={`h-10 sm:h-[65px] w-auto transition duration-300 ${scrolled ? "filter invert-0" : "filter invert"}`}
        />
      </div>
      ...
      <ul className="hidden md:flex space-x-4 sm:space-x-8 text-base sm:text-lg font-medium mr-2 sm:mr-[30px]">
        ...
        {/* Cart & Profile Icons */}
        <div className="hidden md:flex space-x-4 sm:space-x-8 text-xl sm:text-2xl">
          {[
      ...
      {/* Mobile Menu Icon */}
      <div className="md:hidden text-3xl cursor-pointer p-2 rounded-lg hover:bg-white/10 transition-colors duration-200" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
      ...
      <div className={`absolute top-full left-0 w-full md:hidden transition-all duration-300 ease-in-out ${
        ...
        <div className={`${scrolled ? 'bg-white text-black' : 'bg-white text-black'} shadow-2xl rounded-b-2xl py-6 px-4 mx-2 border-t-2 border-pink-200`}>
